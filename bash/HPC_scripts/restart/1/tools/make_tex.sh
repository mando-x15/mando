...
for i in *.graphe; do
	...
	FOFLD=$( echo "scale=3; $NOFLD / $TOTBLACK;" | bc )
	ETADIS2=$FOFLD
	echo "Eta diss: Ld=$LD 0$ETADIS2" >> $LOGS/graspi.$FILENAME.log

	LD=3
	NOFLD=$($QUERYHISTOGRAM $DISTANCES/${i}$DPOLINT $LD)
	FOFLD=$( echo "scale=3; $NOFLD / $TOTBLACK;" | bc )
	ETADIS3=$FOFLD
	echo "Eta diss: Ld=$LD 0$ETADIS3" >> $LOGS/graspi.$FILENAME.log

	LD=4
	NOFLD=$($QUERYHISTOGRAM $DISTANCES/${i}$DPOLINT $LD)
	FOFLD=$( echo "scale=3; $NOFLD / $TOTBLACK;" | bc )
	ETADIS4=$FOFLD
	echo "Eta diss: Ld=$LD 0$ETADIS4" >> $LOGS/graspi.$FILENAME.log
	...
	rm tmp.file
	echo "$TIME $ETADIS1 $ETADIS2 $ETADIS3 $ETADIS4 $ETADIS5 $ETAOUTUPP $ETAOUTPATH $ETAOUTUPPPOL $ETAOUTUPPFULL $INTAREA" >> AllEta.dat
